...
const validateCreateRoom = [
  body('name').trim().isLength({ min: 3, max: 100 }).withMessage('Room name must be between 3 and 100 characters'),
  body('description').optional().trim().isLength({ max: 500 }).withMessage('Description must not exceed 500 characters'),
  body('max_participants').optional().isInt({ min: 2, max: 100 }).withMessage('Max participants must be between 2 and 100')
];
...
// Routes
router.post('/', authenticate, authorize('teacher'), validateCreateRoom, createRoom);
...
router.put('/:room_id/status', authenticate, authorize('teacher'), validateUpdateRoomStatus, updateRoomStatus);
