...
const CodeEditor = ({ roomCode, onLanguageChange }) => {
  ...
  useEffect(() => {
    const handleCodeChange = (event) => {
      const { code: newCode, language: newLanguage, user } = event.detail;
      
      // Only update if it's from another user and language matches
      if (user.id !== socketService.socket?.user?.id && newLanguage === language) {
        setCode(newCode);
  ...
  const runCode = async () => {
    ...
    try {
      ...
      switch (language) {
        ...
        case 'python':
          result = 'Python execution not implemented in demo\nOutput: Hello, World!';
          break;
          
        default:
          result = `${SUPPORTED_LANGUAGES.find(l => l.id === language)?.name || language} code execution not implemented in demo`;
      }
